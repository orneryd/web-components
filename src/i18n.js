...
class I18nMessage extends HTMLElement {
  ...
  get translate() {
    return this.innerHTML || this.getAttribute('key') || this.getAttribute('id');
  }
...
const I18n = new class {
  ...
  addMessages(lang, strings) {
    ...
    const newMessages = {...existing, [lang]: {...existing[lang], ...strings}};
  ...
  getAll(namespace) {
    if (namespace) {
      return Object.entries(this.getMessages()[this._lang]).filter(([key, value]) => {
